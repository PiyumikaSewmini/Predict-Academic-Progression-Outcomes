...
def get_progression():       #to define a new function
    ...
    while True:          #to run infinite time untill the break statement executes
     ...
     ListID.append(StId)
     DictID=dict(zip(ListID,List))      #to join two lists inside the dictionary
     break
...
display_histogram()
with open("StudentID.txt","w") as data:
    data.write(str(DictID))        #to write dictionary into the text file
